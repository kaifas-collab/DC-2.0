...
function startLoop(state: SchedulerState, key: string, fn: () => Promise<void>, intervalMs: number, initialDelayMs: number): void {
  const tick = async (): Promise<void> => {
    try {
      ...
      console.error(`❌ Scheduler loop "${key}" errored (continuing):`, error instanceof Error ? error.message : error)
    } finally {
      if (state.started) {
...
async function workerDrain(cfg: SchedulerConfig): Promise<void> {
  ...
  if (totals.claimed > 0) {
    console.log(
      `🔄 Scheduler worker: ${totals.claimed} claimed across ${totals.batches} batch(es) - ` +
        `${totals.synced} synced, ${totals.deleted} deleted, ${totals.failed} failed, ` +
        `${totals.skippedConfigInvalid} skipped(config), ${totals.deadLettered} dead-lettered`
    )
  }
}
...
async function reconcileSweep(): Promise<void> {
  ...
  if (created > 0 || restaled > 0) {
    console.log(`🧭 Scheduler reconcile: ${created} placement(s) created, ${restaled} re-staled across ${results.length} card(s)`)
  }
}
...
async function validationSweep(): Promise<void> {
  ...
  if (invalid.length > 0) {
    console.warn(`⚠️ Scheduler validation: ${invalid.length}/${outcomes.length} server(s) not valid: ${invalid.map((o) => `${o.server.name}=${o.status}`).join(', ')}`)
  }
}
...
async function downloadTick(cfg: SchedulerConfig): Promise<void> {
  ...
  if (stats) {
    console.log(`⬇️ Scheduler download: ${stats.successful} server(s) ok, ${stats.failed} failed, ${stats.totalCards} total cards mirrored`)
  }
}
...
export function startScheduler(): void {
  ...
  if (!cfg.enabled) {
    console.log('⏸️ Sync scheduler disabled (sync.scheduler.enabled = false) - upload/reconcile will not run automatically')
    return
  }
  ...
  if (cfg.bootstrapOnStart) {
    try {
      ...
      console.log(`🚀 Scheduler startup: registry bootstrapped (${result.servers.length} server(s), ${result.canonicalWatchlists.length} watchlist(s))`)
    } catch (error) {
      console.error('❌ Scheduler startup: registry bootstrap failed (loops still starting):', error instanceof Error ? error.message : error)
    }
  }
  ...
  if (cfg.downloadIntervalMs > 0) {
    ...
    console.log(`✅ Sync scheduler started (worker ${cfg.workerIntervalMs}ms, reconcile ${cfg.reconcileIntervalMs}ms, validation ${cfg.validationIntervalMs}ms, download ${cfg.downloadIntervalMs}ms)`)
  } else {
    console.log(`✅ Sync scheduler started (worker ${cfg.workerIntervalMs}ms, reconcile ${cfg.reconcileIntervalMs}ms, validation ${cfg.validationIntervalMs}ms, download OFF)`)
  }
}
